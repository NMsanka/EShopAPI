...
namespace EShop.API.Services
{
    public class BaseController : ApiController
    {
        ...
        /// <returns></returns>
        protected DateTime GetTimeZone(string CountyCode)
        {
            var serverTime = DateTime.Now;
            //var requestTime = request.DateProperty;
            IEnumerable<string> zoneIds = TzdbDateTimeZoneSource.Default.ZoneLocations.Where(x => x.CountryCode == CountyCode).Select(x => x.ZoneId);

            string windowsZoneId;

            if (CountyCode == "SL") windowsZoneId = OlsonTimeZoneToTimeZoneInfo("Asia/Colombo");
            else windowsZoneId = OlsonTimeZoneToTimeZoneInfo(zoneIds.First());
            if (!string.IsNullOrEmpty(windowsZoneId))
            {
                TimeZoneInfo tst = TimeZoneInfo.FindSystemTimeZoneById(windowsZoneId);
                DateTime tstTime = TimeZoneInfo.ConvertTime(serverTime, TimeZoneInfo.Local, tst);
        ...
        private string OlsonTimeZoneToTimeZoneInfo(string timeInfo)
        {
